...
$(document).on('ready',function(){
				...
				success:function(data,textStatus,jqXHR){ 
					if(data.success){
						if(data.pass){
							...
							email.val('');
							emailCheck.text('the email has been registered');
						}
					}else{
						email.val('');
	...
	$('form').submit(function(event){
		...
		if(!$('#new_email').val() || !$('#verification_code').val()){
			alert('you have not filled in the form completely');
		}else{
			$.ajax('../server/email_reset_process_p2.php',{
				method:'POST',
				data:{
					'email':$('#new_email').val(),
				...
				success:function(data,textStatus,jqXHR){ 
					if(data.success){
						if(!data.pass){
							...
							alert('you have successfully reset your email, please login again');
