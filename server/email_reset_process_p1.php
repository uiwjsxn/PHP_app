<?php
	...
	if(!session_check(['email_code','username'],'valid_user') || !post_check(['email','code'])){
	...
	$pass = true;
	try{
		$pass = (match_user_email($username,$email) ? true : false); //用户名与验证邮箱若不匹配，只返回wrong verification code。不给邮箱错误的提示，以免邮箱泄漏
	...
	ob_clean();
	if($pass){
		unset($_SESSION['email_code']); //下一步会产生新的email_code,当前的email_code失效
